...
import type { AuthResponse, CreateUserDTO, LoginDTO, UpdateUserDTO, UserDataDTO } from "@shared/dto";
...
class UserService {
  ...
  async update(id: number, data: Omit<UpdateUserDTO, 'id'>): Promise<ApiResponse<User>> {
    // Сервер ожидает UpdateUserDTO с id в URL и данными в body
    const response = await instance.patch<ApiResponse<User>>(
      `${this.baseUrl}/${id}`, 
      data
    );
    return response.data;
